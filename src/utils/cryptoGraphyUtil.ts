...
export default class CryptoGraphy implements CryptoGraphyInterface {
    aesModel: string
    token: string
    appId: string
    ...
    KCS7Encoder (textLength: number): string {
      ...
      const pad = String.fromCharCode(amount_to_pad); const s = []
      // eslint-disable-next-line camelcase
    ...
    init (config: { msgSignature: string; timestamp: string; nonce: string; token?: string; appId?: string; encodingAESKey?: string }): CryptoGraphyInterface {
